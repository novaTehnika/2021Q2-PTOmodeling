function [dydt, nonState] = sys_coulombPTOshuntValve(t,y,par)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% sys_coulombPTOshuntValve.m function m-file
% AUTHORS: 
...
%
% UPDATES:
% 12/30/2021 - created from sys_coulombPTOcompressiblePump.m.
% 07/08/2022 - replaced state indice specification in file with seperate
% script, stateIndex_coulombPTOshuntValve.m.
%
...
%% %%%%%%%%%%%%   FUNCTIONS  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function nonState = nonStateVars(t,y,par)
        ...
        % Flow through the shunt valve
         % pressure differential (used multiple time)
        dp_AB = y(iyp_wpA) - y(iyp_wpB);
        
         % valve coefficient, a function of the pressure difference
        kv_sv = kvsv(t);

         % Flow rate calculation, with linearization about zero pressure
         % differential
        nonState.q_sv = (abs(dp_AB) > par.dp_linear)*...
                            orifice(dp_AB,kv_sv) ...
                      + (abs(dp_AB) <= par.dp_linear)*...
                            kv_sv*par.dp_linear_coeff*dp_AB;

        % net flow into of the pumping chambers
        nonState.q_wpA = nonState.q_wpTA - nonState.q_wpAP - nonState.q_sv;
        nonState.q_wpB = nonState.q_wpTB - nonState.q_wpBP + nonState.q_sv;
        ...
        function kv = kvsv(t)
            % Description: calculate the flow coefficient of a shunting
            % valve as a function of time, assuming a trapozoidal profile
            % with some duty within a fixed time period.

            tau = mod(t/par.control.T,1);
            kv = max(par.kv_svMin,par.kv_sv*max(0,min(1, ...
                     1/par.fsv*min(tau-par.control.duty,1-tau) ...
                                                                )));
        end
